...
class DisableAgendaController(
    ...
    fun disableAgenda(@PathVariable agendaId: String, @RequestParam accessKey: String): ResponseEntity<*> = runBlocking {
        with(repository) {
            with(adminRepository) {
                fold(
                    block = { executeCommand(accessKey, agendaId) },
    ...
    context(AgendaRepository, AdminRepository, Raise<DisableAgendaError>)
    private suspend fun executeCommand(accessKey: String, agendaId: String): Agenda {
        withError(block = { validate(AccessKey(accessKey)) }, transform = { raise(ForbiddenAction) })
        return handle(DisableAgendaCommand(agendaId))
    }
